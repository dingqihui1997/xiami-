...
import { Select, Button, Checkbox, Switch, Input, Form, Table, DatePicker, Modal, Upload, Image, Pagination, Popconfirm } from 'antd';
            ...
            >
                ...
                <Form.Item label="商品规格">
                    {Norms && Norms.data && Norms.data.map((item: any, index: number) => {
                        return <div>
                            <div><Checkbox>{item.name}</Checkbox></div>
                            {(item.spec_item.join(',')).map((item1: any, index: number) => {
