                ...
                <Form.Item label="商品规格" className="mal10">
                    {Norms && Norms.data && Norms.data.map((item: any, index: number) => {
                        return <div className="mal20" key={index}>
                            <div className="margin-b20"><Checkbox onChange={choice}>{item.name}</Checkbox></div>
                            <CheckboxGroup options={item.spec_item.split('，'))}  value={chekedlist} onChange={choice1}/>
                            {(item.spec_item.split('，')).map((item1: any, index1: number) => {
