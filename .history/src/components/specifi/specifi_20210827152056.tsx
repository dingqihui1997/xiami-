...
const Specifi = () => {
    ...
    let choice1 = (item: any, e: any) => {//规格项
        ...
        setchekedlist(e);
        setIndeterminate(!!chekedlist!.length && chekedlist!.length < item.spec_item.split('，').length);
        setCheckAll(chekedlist!.length === item.spec_item.split('，').length);
                ...
                <Form.Item label="商品规格" className="mal10">
                    ...
                    {Norms && Norms.data && Norms.data.length ? Norms.data.map((item: any, index: number) => {
                        return <div className="mal20" key={index}>
                            <div className="margin-b20"><Checkbox checked={checkAll && id === item._id} onChange={(e) => choice(item, e)}>{item.name}</Checkbox></div>
