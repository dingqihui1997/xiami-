                ...
                <Form.Item label="商品规格" className="mal10">
                    ...
                    {Norms && Norms.data && Norms.data.length ? Norms.data.map((item: any, index: number) => {
                        return <div className="mal20" key={item._id}>
                            {/* <div className="margin-b20"><Checkbox checked={item.checkList!.length === (item.item.spec_item.split('，')).length} onChange={(e) => changeAllCheck(e, item)}>{item.name}</Checkbox></div> */}
                            {/* {
                                (item.spec_item.split('，')).map((i: string) => {
                                    <Checkbox value={chekedlist}  onChange={(e) => choice1(item, e)} />
                               })
                            }   */}
                            <Divider />
                        </div>
