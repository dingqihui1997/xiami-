...
const Parameter = () => {
    ...
    let onFinish = () => {//验证通过
        ...
        if (!data) {
            ...
            dispatch({//编辑  xx
                ...
                payload: { parentId: parentId, id: data._id, url: values.url, name: values.name, desc: values.desc }
            })
        }
        setVisible(false)
    }
    ...
    let confirm = (text: any, record: any) => {//确认删除
        console.log(record);
        dispatch({
            type: 'Params/delParams',
            payload: { parentId: parentId, attrId: record._id }
        })
    }
    ...
    return (
        <div>
            <Card>
                ...
                <Modal visible={visible} title={data ? '编辑商品参数' : '添加商品参数'} onOk={handleOk}
