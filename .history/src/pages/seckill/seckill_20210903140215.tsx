...
const Seckill = () => {
    ...
    let [good, setgood] = useState<any>({})//商品good
    ...
    let choice = (e: string, good1: any) => {//选择商品
        setgoodsId(e)
        goodsname(good1.children)
        let arr: any = {}
        arr = goods && goods.data.find((item: any, index: number) => {
            return item._id === good1.value
        })
        setgood(arr)
    }
    let handleOk = () => {//表单验证成功
        ...
        if (data) {//编辑
            dispatch({
                ...
                payload: { start_time: values.start_time, end_time: values.end_time, goods_number: values.goods_number, goods: good, isShow: true, current: current, pageSize: pageSize }
            })
        } else {
            dispatch({
    ...
    let confirm = (text: any, record: any) => {//确认删除
        ...
        dispatch({
            ...
            payload: { id: record._id, goodsId: record.goods._id, current: current, pageSize: pageSize, }
        })
    }
    //时间
    let submit = () => {
        ...
        <div>
            <Card>
                ...
                <Modal visible={visible} title={data && data ? '编辑秒杀' : '添加秒杀'} onOk={submit}
                    ...
                    <div className="mal20 margin-b10"> 商品原价为{price && price.originalPrice}</div>
                    <Form
                        form={form}
                        preserve={false}
                        onFinish={handleOk}
                    >
                        <Form.Item
                            ...
                            label="秒杀商品"
                            name="goods"
                            initialValue={data && data ? data.goods && data.goods._id : ''}
                            rules={[{ required: true, message: '秒杀商品不能为空' }]}
                        >
                            <Select onChange={choice} placeholder="请选择商品">
