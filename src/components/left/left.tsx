        ...
        <div>
            ...
            >
                {data.map((item: any, index: number) => {
                    if (index === 5) {
                        return <SubMenu key={item.url} title={intl.formatMessage({ id: item.t })} icon={<item.icon />}>
                            {item.children && item.children.map((item1: any, index: number) => {
                                return <Menu.Item key={item1.url} icon={<item1.icon />} >{intl.formatMessage({ id: item1.t })}</Menu.Item>
                            })}
